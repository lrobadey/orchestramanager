  ...
  return (
    <div className="home-stratum floor">
      <div className="floor-grid">
        <RosterColumn
          sections={sectionStrengths}
          composite={compositeStrength}
          watch={watchPrincipals}
        />
        <NextConcertColumn
...
function RosterColumn({ sections, composite, watch }: RosterColumnProps) {
  return (
    <div>
      ...
      <div className="roster-watch">
        ...
        {watch.map(p => (
          <div key={p.id} className="roster-watch-row">
            <span>
              <span className="roster-watch-name">{p.name}</span>
              <span className="roster-watch-position">{p.position}</span>
            </span>
...
function NextConcertColumn({
  ...
  return (
    <div>
      ...
      <div className="next-title-line">
        <span className="accent">{slotName}</span>
      </div>
      <div className="next-meta">
        {VENUE_NAME} · {seasonComplete ? '—' : date}
      </div>
        ...
        {selectedWorks.slice(0, workCount).map((w, i) => (
          <div key={i} className="next-slot-row">
            ...
            <div>
              {w ? (
                <>
                  <span className="next-slot-title">
                    <span className="next-slot-composer">{w.composer},</span>{' '}
                    <span style={{ fontStyle: 'italic' }}>{w.title}</span>
                  </span>
                  <div className="next-slot-hint">
                    {w.durationMinutes}m · P{w.artisticPrestige} · D{w.audienceDraw} · L{w.rehearsalLoad}
                  </div>
                </>
              ) : (
                <>
                  <span className="next-slot-title empty">— open —</span>
                  <div className="next-slot-hint">{SLOT_HINTS[i] ?? 'Open slot'}</div>
                </>
              )}
            </div>
      ...
      <div className="next-suggested">
        ...
        {suggested.map(w => (
          <div key={w.id} className="next-suggested-row">
            <span>
              <span className="next-suggested-name">{w.composer},</span>{' '}
              <span className="next-suggested-title">{w.title}</span>
            </span>
            ...
            <span className={`next-suggested-stat${w.rehearsalLoad > 60 ? ' ember' : ' muted'}`}>
              L{w.rehearsalLoad}
            </span>
          </div>
        ))}
  ...
  return (
    <div>
      <div className="floor-col-head">
        ...
        <span className="hc-num" style={{ fontSize: 9, color: 'var(--bark)' }}>
          {INBOX_MESSAGES.length} STUB
        </span>
      </div>
      ...
      <div className="finance-trace">
        <div className="finance-trace-head">
          <span className="hc-eyebrow">Finance · 14 wk trace (stub)</span>
          <span className="hc-num" style={{ fontSize: 9.5, color: 'var(--pine)' }}>
            +${netK}K net
          </span>
        </div>
        <div className="hc-rule-silver" style={{ marginBottom: 8, opacity: 0.5 }} />
        <svg
          className="finance-spark"
          viewBox="0 0 300 44"
          preserveAspectRatio="none"
          role="img"
          aria-label="Finance sparkline (stub)"
        >
          <line x1="0" y1="34" x2="300" y2="34" stroke="var(--hairline)" strokeWidth="0.5" />
          <polyline
            points={FINANCE_SPARKLINE.map(
              (v, i) => `${(i / (FINANCE_SPARKLINE.length - 1)) * 300},${42 - v * 1.4}`,
            ).join(' ')}
            fill="none"
            stroke="var(--silver)"
            strokeWidth="1.2"
          />
          {FINANCE_SPARKLINE.map((v, i) => (
            <circle
              key={i}
              cx={(i / (FINANCE_SPARKLINE.length - 1)) * 300}
              cy={42 - v * 1.4}
              r="1.4"
              fill="var(--silver)"
            />
...
// Derive a pseudo-stress score (0–100) from section strength so the home
// readout matches the new design's STR field. Real per-program stress is in
// calculateSectionStress, but here we want a roster-only at-rest reading.
...
function pickWatchPrincipals(principals: Principal[]): Principal[] {
  // The 3 most-watched principals = lowest combined form/morale.
  return [...principals]
...
function suggestWorks(works: Work[], usedIds: Set<string>, n: number): Work[] {
  return [...works]
    ...
    .sort(
      (a, b) =>
        b.audienceDraw + b.artisticPrestige - (a.audienceDraw + a.artisticPrestige),
    )
    .slice(0, n)
}
